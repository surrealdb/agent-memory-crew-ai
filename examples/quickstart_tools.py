...
def main() -> None:
    ...
    # environment: get_agent_memory_tools(scope="user/tobie").
    tools = get_agent_memory_tools(config=config, client=fake, scope="user/tobie")
    by_name = {t.name: t for t in tools}

    print("tools:", ", ".join(by_name))

    print("\nremember ->", by_name["agent_memory_remember"].run(text="Tobie was promoted to CTO"))
    print("remember ->", by_name["agent_memory_remember"].run(text="Tobie moved to Lisbon"))

    print("\nrecall ->\n" + by_name["agent_memory_recall"].run(query="what about tobie?"))
    print("\ncontext ->", by_name["agent_memory_context"].run(query="summarise what you know"))
    print("\nreflect ->", by_name["agent_memory_reflect"].run(query="this week", persist=True))

    print("\nTo use these on a real agent:")
    print("    from crewai import Agent")
    print("    agent = Agent(role='Analyst', tools=get_agent_memory_tools(scope='user/tobie'))")
